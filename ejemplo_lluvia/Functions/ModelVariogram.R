### Equipo 2
# Arreola Romero​ Karen
# Castelblanco Rivera Luis Felipe
# Hernández Guillén Carmen
# López Jiménez Marcos Emiliano
# Martínez Pérez Paola 
          ...
          NomP1, NomP2, NomP1P2)
{
  ...
  ambas <- gstat(ambas, id = "Prop2", formula = Prop2 ~ 1, data = Datos)
  ...
  ########################## First plot #################################
  VMod_1 <- (Sill1 + Nugget1) - cov.spatial(v.cross$dist[(k0 + k1 + 1):(k0 + k1 + k2)], cov.model = Modelo1,
                                      cov.pars = c(Sill1, Alcance))
  
  Error2_1 <- (v.cross$gamma[(k0 + k1 + 1):(k0 + k1 + k2)] - VMod_1)^2
  SumaError_1 <- sum(Error2_1)
  ...
  legend(0.7 * MaxX1, 0.5 * MaxY1, c("Sill+Nugget", "Range",
                                     "Variance"), col = c("Blue", "green", "Red"), lty = c(3,
                                                                                           5, 1), bty = "n")
  title(paste(NomP1,"   MSE: ", sprintf("%.4f", SumaError_1)), cex.main = 1.5)
  
  
  #################################### Third Plot  ########################################
  VMod_3 <- (Sill3 + Nugget3) - cov.spatial(v.cross$dist[1:k0], cov.model = Modelo1,
                                            cov.pars = c(Sill3, Alcance))
  
  Error2_3 <- (v.cross$gamma[1:k0] - VMod_3)^2
  SumaError_3 <- sum(Error2_3)
  ...
  abline(v = Alcance, lty = 5, col = "orange", lwd = 2)
  legend(0.7 * MaxX2, 0.5 * MaxY2, c("Sill+Nugget", "Range"),
         col = c("Blue", "orange"), lty = c(3, 5), bty = "n")
  title(paste(NomP1P2,"   MSE: ", sprintf("%.4f", SumaError_3)), cex.main = 1.5)
  
  
  ##########            Second Plot           ###########################################
  VMod_2 <- (Sill2 + Nugget2) - cov.spatial(v.cross$dist[(k0 + 1):(k0 + k1)], cov.model = Modelo1,
                                            cov.pars = c(Sill2, Alcance))
  
  Error2_2 <- (v.cross$gamma[(k0 + 1):(k0 + k1)] - VMod_2)^2
  SumaError_2 <- sum(Error2_2)
  par(mar = c(5, 4, 2, 1))
  plot(v.cross$dist[(k0 + 1):(k0 + k1)], v.cross$gamma[(k0 +
                                                          1):(k0 + k1)], xlab = "Distance", ylab = "Semivariance",
       xlim = c(0, MaxX3), ylim = c(0, MaxY3), pch = 21, col = "orange",
       bg = "blue", cex.lab = 1.3, cex.axis = 1.2)
  grid(col = "lightgray", lty = "dashed", lwd = par("lwd"), equilogs = TRUE)
  par(new=TRUE)
  plot(v.cross$dist[(k0 + 1):(k0 + k1)], v.cross$gamma[(k0 +
                                                          1):(k0 + k1)], xlab = "Distance", ylab = "Semivariance",
       xlim = c(0, MaxX3), ylim = c(0, MaxY3), pch = 21, col = "pink",
       bg = "blue", cex.lab = 1.3, cex.axis = 1.2)
  ...
  abline(v = Alcance, lty = 5, col = "pink", lwd = 2)
  legend(0.7 * MaxX3, 0.5 * MaxY3, c("Sill+Nugget", "Range",
                                     "Variance"), col = c("Blue", "pink", "Red"), lty = c(3,
                                                                                           5, 1), bty = "n")
  title(paste(NomP2,"   MSE: ",sprintf("%.4f", SumaError_2)), cex.main = 1.5)
}
